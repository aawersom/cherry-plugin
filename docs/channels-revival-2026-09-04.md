...
> **Статус (v0.13.20, тот же день):** §3.1 ebun и §3.2 huyamba сделаны и проверены на стенде (внутренний плеер ebun: currentTime 4.2 с вместо «Format error»; huyamba: 20 карточек, 100 % постеров/клипов/длительностей, категория/поиск фильтруют, стр. 2 новая, стрим MP4 с IP устройства, внутренний плеер 14.8 с за 20 с); 24rolika скрыт с плиток (`disabled: true`, `_activeSources`). Дальше по порядку: xhamster → ebalovo → RU-пачка.
