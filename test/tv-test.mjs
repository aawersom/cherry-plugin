...
code = code.slice(0, i) + '\n;try{window.__CHERRY={SOURCES:SOURCES,cherryFetch:cherryFetch,buildProxyUrl:buildProxyUrl,_isAndroid:_isAndroid};}catch(e){window.__CHERRY_ERR=String(e);}\n' + code.slice(i);
...
console.log('id            cards1/2  total emptyTitles dupP1P2  stream            host');
...
for (const cid of ids) {
  ...
  const flag = (r.emptyTitles > 0 ? ' ⚠TITLES' : '') + (r.dupP1P2 > 0 ? ' ⚠DUP' : '') + (r.cards2 === 0 && r.cards1 > 0 ? ' ⚠NOPAGE2' : '') + (/EMPTY|PROTO/.test(r.streamKind || '') ? ' ⚠STREAM' : '');
  console.log(`${cid.padEnd(13)} ${String(r.cards1).padStart(3)}/${String(r.cards2).padEnd(3)}   ${String(r.total).padStart(4)}  ${String(r.emptyTitles).padStart(6)}    ${String(r.dupP1P2).padStart(4)}   ${(r.streamKind||'-').padEnd(13)} ${(r.streamHost||'').padEnd(22)}${flag}`);
}
ws.close();
process.exit(0);
